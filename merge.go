...
func mergeForms(form, form2 *pdf.PdfAcroForm, docName string) (*pdf.PdfAcroForm, error) {
...
func mergePdf(inputPaths []string, outputPath string) error {
	...
	final_doc := len(inputPaths)-1
	page_marked := make(map[int]bool)
	
	for docNum, inputPath := range inputPaths {
		...
		for _, field := range form_values {
			page_marked[field.Page] = true
			this_marker_marked[field.Page] = true
			marker_initials = field.Marker
		}
		
		//pdfextract.PrettyPrintStruct(page_marked)
